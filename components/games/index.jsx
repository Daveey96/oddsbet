...
export default function GameLayout({ gGames, index, last }) {
    ...
    >
      ...
      >
        <span className="max-w-[90%] text-base gap-1.5 flex items-center pl-4">
          ...
          <List
            className={"w-full"}
            iClass="pt-0.5 pb-0.5 mt-0.5 text-[13px] gap-1 pl-2"
            activeClass={`text-c2 bg-c2/5 rounded-lg pr-3`}
            inActiveClass={"bg-white/5 rounded-lg pr-2"}
            onActive={true}
            onClick={changeSport}
            list={sports}
            v="id"
